...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        # transform 200 => 768
        assert input_inter_mask is not None # input_inter_embeddings [N, L, W, D] [4, 512, 5, 200]
        ...
        inter_word_outputs = self.dropout(inter_word_outputs)    # [4, 512, 5, 768]
        
        inter_word_outputs = inter_word_outputs.mean(-2) 
        # print(inter_word_outputs.shape)
        attention_output = attention_output + inter_word_outputs
...
class ZWCBertModel_v2(BertPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True):
        super(ZWCBertModel_v2, self).__init__(config)
