...
class null_model(nn.Module):
    
    def __init__(self):
        super(null_model, self).__init__()
    ...
    def loss(self, input, mask, labels):
        
        sunLoss = torch.tensor(0.0).cuda()
        for i in range(input.size(0)):

            x = input[i][mask[i]]
            y = labels[i][mask[i]]
            sunLoss += self.lossfct(x,y)
        sunLoss = sunLoss / input.size(0)
        return sunLoss
...
class NERTrainer(ITrainer):
    ...
    def model_init(self, **args):
        ...
        self.model, self.birnncrf, self.crf = self.bert_ner()
        if self.classify == 'lstm_crf':
            self.bc = self.birnncrf
        elif self.classify == 'crf':
            self.bc = self.crf
        elif self.classify == 'softmax':
            self.bc = null_model()
    ...
    def train(self, resume_path=False, resume_step=False, lr1=2e-5, lr2=1e-3):
        ...
        if not resume_path == False:
            ...
            if self.classify == 'lstm_crf':
                self.bc = torch.load(os.path.join(
                    resume_path, 'lstm_crf', 'lstm_crf_{}.pth'.format(resume_step)))
            elif self.classify == 'crf':
                self.bc = torch.load(os.path.join(
                    resume_path, 'crf', 'crf_{}.pth'.format(resume_step)))
            self.model.to(device)
            self.bc.to(device)
        ...
        for epoch in range(self.num_epochs):
            ...
            for it in train_iter:
                ...
                if self.classify != 'softmax':
                    pred = self.bc(hidden_states, it['input_ids'].gt(0))[1]
                elif self.classify == 'softmax':
                    pred = self.bc(hidden_states, it['input_ids'].gt(0)).tolist()
    ...
    def save_model(self, current_step=0):
        ...
        if self.classify == 'lstm_crf':
            ...
            if not os.path.exists('./save_model/{}/crf'.format(dir)):
                os.makedirs('./save_model/{}/crf'.format(dir))
        torch.save(
        ...
        if self.classify == 'lstm_crf':
            ...
            torch.save(self.bc,
                    './save_model/{}/crf/crf_{}.pth'.format(dir, current_step))
